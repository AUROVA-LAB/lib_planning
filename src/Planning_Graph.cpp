...
void Planning_Graph::addNode(vector<double> coordinates,vector<vector<double> >  matrix){
	Node n(coordinates, matrix);
	nodes.push_back(n);
}

void Planning_Graph::addNode(long id,vector<double> coordinates,vector<vector<double> >  matrix){
...
void Planning_Graph::addDynamicPosition(vector<double> coordinates,vector<vector<double> >  matrix){
...
void Planning_Graph::addLinkBetweenNodes(vector<double> coordinates1,vector<double> coordinates2){
	...
	if(pos1!=-1 && pos2!=-1){
		...
		l1->setEuclideanDistance(nodes[pos2].calculateEuclideanDistance(nodes[pos1]));
		l1->setMahalanobisDistance(nodes[pos2].calculateMahalanobisDistance(nodes[pos1]));
...
Node Planning_Graph::getNextNode(Position initPos){
	...
	if(nodes.size()>0){
		...
		nextNode.setYaw(calculateSense(actualPosition,nextNode,evaluateNextNode(nextNode)));
	}else
	{
		nextNode=finalGoal;
	}
...
Node Planning_Graph::evaluateNextNode(Position initPos){
	...
	for(int i=0; i<initNodes.size(); i++){
		for(int j=0; j<endNodes.size(); j++){
			...
			if(distance > radiusVehicle){
				...
				distance += calculateDistanceNodes(initNodes[i],endNodes[j]);
	...
	calculateDistanceNodes(nextNode,finalNode);


	Node endNode(finalGoal);
	double distanceToEndPos = distanceNodeAndPosition(initPos,endNode);
	double endNodeDistance= distanceNodeAndPosition(initPos,getCloserNode(finalGoal));
...
bool Planning_Graph::existLinkBetweenNodes(Node node1,Node node2){
	...
	for(int j=0;j<n1.getLinks().size();j++){
		if(n1.getLinks()[j]->nodes[0]->equals(n2) || n1.getLinks()[j]->nodes[1]->equals(n2)){
			exist=true;
		}
	}
	return exist;
...
void Planning_Graph::dynamicPositionsToNodes(){
	...
	if(dynamicPositions.size()>2){
			...
			if(close){
				if(lastNodeVisited.isValid() && !closerNode.equals(lastNodeVisited)){
					if(!existLinkBetweenNodes(closerNode,lastNodeVisited)){
						addLinkBetweenNodes(lastNodeVisited.getCoordinates(),closerNode.getCoordinates());
					}
				}
		...
		if(!close && (orientation >=minimumDegreesNodes || orientation<=(-minimumDegreesNodes))){
			Node newNode(dynamicPositions[i].getCoordinates(),dynamicPositions[i].getMatrix());
			addNode(newNode);
			if(lastNodeVisited.isValid()){
				addLinkBetweenNodes(lastNodeVisited.getCoordinates(),newNode.getCoordinates());
			}
...
double Planning_Graph::calculateSense(Node pos1,Node pos2,Node pos3){
	...
	double degreeXY = atan2(vector2[1],vector2[0]) - atan2(vector1[1],vector1[0]);
...
double Planning_Graph::calculateDistanceNodes(Node initNode, Node endNode){
	...
	// Visit all nodes
	while(!allNodesSeen){
		path[selectNode]->seen = true;
		// Find the nearest neighbor
		for(int j=0; lastNode.getLinks().size() > j; j++){
			double distanceBetweenNodes = lastNode.getLinks()[j]->distance;
			int newDistance = distanceBetweenNodes + lastNode.distance;

			Node *childNode;
			if(*lastNode.getLinks()[j]->nodes[0] == lastNode) {
				childNode = lastNode.getLinks()[j]->nodes[1];
			}else{
				childNode = lastNode.getLinks()[j]->nodes[0];
			}
